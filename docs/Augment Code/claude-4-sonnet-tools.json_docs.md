# Documentation: claude-4-sonnet-tools.json

## File Metadata

- **Path**: `Augment Code/claude-4-sonnet-tools.json`
- **Size**: 28992 bytes (28.31 KB)
...
This file contains 515 lines of json content.

### JSON Structure

This JSON file contains a dictionary with 1 top-level keys:
...
with open('Augment Code/claude-4-sonnet-tools.json', 'r') as f:
    data = json.load(f)
```
...
```bash
python -c "import json; json.load(open('Augment Code/claude-4-sonnet-tools.json'))"
# or
jq . 'Augment Code/claude-4-sonnet-tools.json'
```
